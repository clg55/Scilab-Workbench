...
c     store intermediate result
         ...
         stk(lys)=tleft
         call dcopy(ny+nd,stk(ly),1,stk(lys+1),1)
...
c     lsodar: a root found
            ...
            if(istore.eq.1) then
               if(it.eq.0) then
                  lys=lyp+(k-1)*(ny+nd)
                  call dcopy(ny+nd,stk(ly),1,stk(lys),1)
               else
                  lys=lyp+(k-1)*nys2
...
c      
      ...
      if(single) then
         ...
         inc=1
         if(ly.gt.lyp) inc=-1
         call dcopy((ny+nd+1)*nn,stk(lyp),inc,stk(ly),inc)
         ...
         if(ly.gt.lyp) inc=-1
c
         call dcopy((ny+nd)*nn,stk(lyp),inc,stk(ly),inc)
