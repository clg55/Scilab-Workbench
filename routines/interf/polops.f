...
c     
 ...
 31   continue
   ...
   35 continue
      ...
      if(it3.eq.1) call dcopy(vol,stk(l3i),1,stk(l1r+vol),1)
...
c
 ...
 71   continue
      ...
      do 73 i=1,mn
         ...
         if(nexp.gt.0) then
            call dcopy(ni,stk(l1),1,stk(l),1)
            if(it1.eq.1) call dcopy(ni,stk(l1+vol),1,stk(l+nr),1)
         else
            stk(l)=1.0d0
 ...
 73   continue
      ...
      l1=sadr(id1+mn+1)
      call dcopy(nr*(it1+1),stk(lr),1,stk(l1),1)
...
c     set output variable
      ...
      l1=sadr(il1+mr*nr+9)
      call dcopy(volr*(itr+1),stk(lr),1,stk(l1),1)
...
c     .  arg2(:), just reshape to column vector
         ...
         l1=sadr(il1+9+mn2)
         call dcopy(volr*(it2+1),stk(l2r),1,stk(l1),1)
...
c     form resulting variable
      ...
      l1=sadr(il1+9+mi)
      call dcopy(volr*(it2+1),stk(lr),1,stk(l1),1)
      lstk(top+1)=l1+volr*(it2+1)
      go to 999
c
 132  continue
...
c      a [:b]:c
     ...
     $           var1(3).ne.istk(il1+6).or.var1(4).ne.istk(il1+7)) then
      ...
      l=sadr(il1+12)
      call dcopy(n1+n2+n3,stk(lw),1,stk(l),1)
