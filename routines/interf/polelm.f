...
c     coeff of a matrix of polynomials
 ...
 30         continue
            ...
            call dcopy(nv,stk(lv),1,stk(lw),1)
            lv=lw
 ...
 41         continue
            l=sadr(ilr+4)
            if(.not.ref) call dcopy(mn1*nv*(it1+1),stk(lr),1,stk(l),1)
            istk(ilr)=1
            istk(ilr+1)=m1
            istk(ilr+2)=n1*nv
            istk(ilr+3)=it1
     ...
     $              ,stk(lstk(top)),1)
         ...
         do 50 k=1,nv
            if(stk(lv-1+k).eq.0.0d0) then
               call dcopy(mn1,stk(l1),1,stk(lr+(k-1)*mn1),1)
               if(it1.eq.1) call dcopy(mn1,stk(l1+mn1),1,
...
c
 ...
 57   continue
      if(ref) then
         ...
         istk(ilr+9)=1+maxd
         l1=sadr(il1+10)
         call dcopy(maxd*(it1+1),stk(lr),1,stk(l1),1)
         lr=l1
      endif
...
c     move d and copy value of n in place of its reference
         ...
         call dcopy(vold,stk(lstk(top)),-1,stk(lstk(top-1)+voln),-1)
         call dcopy(voln,stk(lstk(k)),1,stk(lstk(top-1)),1)
...
c      copy value of b in place of its reference
         ...
         call dcopy(vold,stk(lstk(k)),1,stk(lstk(top)),1)
         lstk(top+1)=lstk(top)+vold
      endif
      lw=lstk(top+1)
...
c     move b and copy value of a in place of its reference
         ...
         call dcopy(volb,stk(lstk(top)),-1,stk(lstk(top-1)+vola),-1)
         call dcopy(vola,stk(lstk(k)),1,stk(lstk(top-1)),1)
...
c      copy value of b in place of its reference
         ...
         call dcopy(volb,stk(lstk(k)),1,stk(lstk(top)),1)
         lstk(top+1)=lstk(top)+volb
      endif
      lw=lstk(top+1)
...
c
      if(refr) then
         ...
         call dcopy(vol,stk(lstk(k)),1,stk(lstk(top)),1)
...
c     b matrice de polynome
         ...
         idb1=il+8
         l=sadr(il+9+mna)
         call dcopy(istk(idb1+mna)-1,stk(lbr),1,stk(l),1)
         l=l+istk(idb1+mna)-1
      else
c     b matrice de scalaires
         call icopy(4,istk(ilb),1,istk(il),1)
...
c     copy 4th entry of the list 
      ...
      call dcopy(mb,stk(ll+istk(ilr+5)-1),1,stk(l),1)
      l=l+mb
      lstk(top+1)=l
...
c     
         ...
         do 88 i1=0,mn1-1
            ...
            if(mij.gt.0) call dcopy(mij,stk(lij+n1-1),1,stk(l2+i1),mn1)
 88      continue
c     
         maxit=maxit+n1
...
c     .  convert polynomial matrix to matrix of polynomials 
         ...
         l1=lr
         do 89 i=0,mn1-1
            call dcopy(n1,stk(l2+i),mn1,stk(l1),1)
            l1=l1+n1
 89      continue
         do 90 i=0,mn1
            istk(id1+i)=1+n1*i
