...
c      numerateur (complexe)
      ...
      istk(il+2)=1
      istk(il+3)=it
      call dcopy(na*(it+1),stk(lr),1,stk(la),1)
...
c      denominateur (reel)
      ...
      istk(il+2)=1
      istk(il+3)=it
      call dcopy(na,stk(lb),1,stk(l),1)
      lstk(top+1)=sadr(il+4)+na*(it+1)
      return
c
...
c     paire complexe
      call dcopy(na,stk(lzr2),1,stk(lzi),1)
      do 23 j=1,na
      stk(lzi2+j-1)=-stk(lzi+j-1)
  23  continue
      call dcopy(na,stk(lzr),1,stk(lzr2),1)
      kk=kk+2
      goto 25
  24  continue
c     cas reel
  ...
  22  continue
  26  continue
      call dcopy(nn*(it+1),stk(lz),1,stk(l),1)
...
c     creation d'une structure pour l'external
     ...
     x        ,eps,ndim,fail,istk(ilw))
      ...
      top=top0+1
      if(lhs.eq.2) then
         call dcopy(nn,stk(lz),1,stk(la),1)
         stk(le)=ndim
         istk(ile+1)=1
         istk(ile+2)=1
         return
...
c     creation de la variable z
      ...
      istk(ilz+3)=0
      l=sadr(ilz+4)
      if(macro) call dcopy(na*na,stk(lz),1,stk(l),1)
      lstk(top+1)=l+nn
...
c
        call ereduc(stk(l1),n1,m1,stk(lw4),stk(lw5),stk(lw6),stk(lw7),st
     & k(l2))
        if (err .gt. 0) return
c
        top=top-rhs
...
c     
        ...
        lw=sadr(ilw+4)
        call dcopy(n1*m1,stk(l1),1,stk(lw),1)
        lw=lw+n1*m1
        lstk(top+1)=lw-mv
        endif
c     
...
c     
        ...
        istk(ilw+3)=0
        lw=sadr(ilw+4)
        call dcopy(n1*n1,stk(lw4),1,stk(lw),1)
        lw=lw+n1*n1
        lstk(top+1)=lw-mv
        endif
c     
...
c     
        ...
        istk(ilw+3)=0
        lw=sadr(ilw+4)
        call dcopy(m1*m1,stk(lw5),1,stk(lw),1)
        lw=lw+m1*m1
        lstk(top+1)=lw-mv
        endif
c     
...
c
        call fstair(stk(l1),stk(l2),stk(l3),stk(l4),n1,m1,istk(iadr(l5))
     & ,istk(iadr(l6)),stk(l7),stk(lw10),stk(lw11),stk(lw12),stk(lw13),
     & stk(lw14),stk(lw15),stk(lw16),stk(lw17),ierr)
        if (ierr.ne.0) then
           buf='fstair failed !'
           call error(9999)
           return
...
c     
        ...
        lw=sadr(ilw+4)
        call dcopy(n2*m2,stk(l2),1,stk(lw),1)
        lw=lw+n2*m2
        lstk(top+1)=lw-mv
        endif
c     
...
c     
        ...
        lw=sadr(ilw+4)
        call dcopy(n3*m3,stk(l3),1,stk(lw),1)
        lw=lw+n3*m3
        lstk(top+1)=lw-mv
        endif
c     
...
c     
        ...
        lw=sadr(ilw+4)
        call dcopy(n4*m4,stk(l4),1,stk(lw),1)
        lw=lw+n4*m4
        lstk(top+1)=lw-mv
        endif
c     
