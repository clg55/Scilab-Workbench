...
c     checking variable sp
     ...
     $              istk(il1+5),istk(iw))
      ...
      if(fin.eq.13) then
c    SCILAB function : [ac,p]=spchol(a) / a=p*ac*ac'*p'
c-----------------------------------------------------
         lw = lstk(top+1)
         l0=  lstk(top+1-rhs)
         if (rhs .ne.1) then
...
C
 ...
 32      continue
     ...
     &    'WARNING: Possible matrix is not positive definite')
        ...
        do 33 i=1,n+nnzl
  33    istk(iindn+n+nnzl-i)=istk(iindlnz+n+nnzl-i)
        iindlnz=iindn
        do 34 i=1,nnzl
  34    stk(iln+nnzl-i)=stk(ilnz+nnzl-i)
        ilnz=iln
        do 35 i=1,n+1
  35    istk(ixln+n+1-i)=istk(ixlnz+n+1-i)      
        ixlnz=ixln
         call icopy(n,istk(iinvp),1,istk(iindlnz-n),1)
         err = sadr(iindlnz+n+nnzl)-lstk(bot)
           if (err .gt. 0) then
