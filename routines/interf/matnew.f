...
c     on sort aussi q
      ...
      istk(iq+3)=0
      lqnew=sadr(iq+4)
      call dcopy(nna,stk(lq),1,stk(lqnew),1)
...
c placement de poles
     ...
     2    stk(lw1),stk(lw2),stk(lw2+n),stk(lw2+2*n),stk(lw2+2*n+m))
      ...
      istk(ia+1)=m
      call dcopy(n*m,stk(lg),1,stk(la),1)
      lstk(top+1)=la+n*m
      goto 99
c
   91 call error(86)
...
c     sortie des resultats
      ...
      istk(ia+2)=min(nu,1)
      istk(ia+3)=1
      call dcopy(nu,stk(lr),1,stk(la),1)
      call dcopy(nu,stk(li),1,stk(la+nu),1)
      lstk(top+1)=la+nu+nu
      top=top+1
      ib=iadr(lstk(top))
      lb=sadr(ib+4)
      ...
      istk(ib+2)=min(nu,1)
      istk(ib+3)=0
      call dcopy(nu,stk(lk2),1,stk(lb),1)
      lstk(top+1)=lb+nu
      if(lhs.eq.3) then
         top=top+1
         ib=iadr(lstk(top))
...
c
   ...
   42 continue
      ...
      call dcopy(igran,stk(lwgr),1,stk(la),1)
      istk(ia) = 1
      istk(ia+1) = q
      istk(ia+2) = p*ido
      istk(ia+3) = itfr
...
c
   ...
   48 continue
      ...
      istk(il+3)=itf
      l=sadr(il+4)
      call dcopy(md*nd*nf*(itf+1),stk(lw),1,stk(l),1)
...
c
   51 lwgr = lw + n
      ...
      if(rhs.eq.4) then
                   lx=sadr(ix+4)
                   call dcopy(n,stk(lx),1,stk(lwgr),1)
                   else
   ...
   52 continue
      if(lhs.eq.1) goto 53
      call dcopy(n,stk(lwgr+ig),1,stk(la),1)
      lstk(top+1)=la+n
      istk(ia+1)=n
      istk(ia+2)=1
      top=top+1
   53 iy=iadr(lstk(top))
      ...
      istk(iy+3)=0
      ly=sadr(iy+4)
      call dcopy(ido*n,stk(lwgr),1,stk(ly),1)
      lstk(top+1)=ly+n*ido
      goto 99
c
...
c
      ...
      istk(iy+3)=0
      ly=sadr(iy+4)
      call dcopy(ido*nd,stk(lr),1,stk(ly),1)
