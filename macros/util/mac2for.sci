...
argld=[];argli=[],locals=[]
for iv=1:na
  var=vnms(iv,1);vartyp=vtps(iv)
  if part(var,1:4)<>'work'&part(var,1:5)<>'iwork' then
    nl=vartyp(2);nc=vartyp(3);it=vartyp(4)
    if type(nl)=1 then nl=string(nl),end
    if type(nc)=1 then nc=string(nc),end
    if nl+nc='11' then
      if it=0 then
         nm=var
      else
    ...
    if vartyp(1)='1' then
      argld=[argld;nm]
      com='c      '+var+' : double precision variable'+com
    else
      argli=[argli;nm]
      com='c      '+ var+' : integer variable'+com
    end
    if iv<=bot then
      hdr=[hdr;com],
    end
