...
is ``loaded'' into the Scilab environment and is used as follows.
\begin{verbatim}
--> exists('fact')
 ans       =
...
{\tt exists}\index{macros!{\tt exists}}\index{exists@{\tt exists}} 
indicates that
...
calculates $5!$. Finally, the function is compiled using {\tt comp}
for faster execution. Note that compiling {\tt fact} can be realized directly
by the command {\tt getf('../macros/fact.sci','c')}.
...
the three vertical dots represent the list of instructions performed by
the function.  Inputs and ouputs parameters can be {\it any} Scilab object
...
the command {\tt getf(filename,'c')}. If the file {\tt filename} contains the
function {\tt foo}, the function {\tt foo} can be executed only if
it has been previously loaded by the command {\tt getf(filename,'c')} 
(where {\tt 'c'} is optional).
A file may contain several functions. Functions can also be defined ``on line''
...
The following example loads a function called {\tt foo} into Scilab which 
