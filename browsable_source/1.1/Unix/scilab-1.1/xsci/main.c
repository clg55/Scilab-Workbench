...
static char *fallback_resources[] = {
  ...
  "XScilab*tekMenu.Label:  Tek Options (no app-defaults)",
...
/* JPC : term est ds vpane et plus ds toplevel 
    VTinit est change en VTinit1(); 
    il y a un probleme ds le XtRealize car la place gardee pour 
    xterm est trop petite, mais je crois que c'ets lie a un bug 
    de xterm (voir XXX this is bogus dans le fichier charproc.c
    voir Createsub
*/
 ...
 */
{
	...
	wm_delete_window = XInternAtom(XtDisplay(toplevel), "WM_DELETE_WINDOW",
				       False);
	if (!screen->TekEmu)
	    VTInit1(toplevel);		/* realize now so know window size for tty driver */
#ifdef TIOCCONS
	if (Console) {
	    /*
