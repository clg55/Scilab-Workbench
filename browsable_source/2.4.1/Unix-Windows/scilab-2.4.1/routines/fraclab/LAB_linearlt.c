...
{ 
  ...
  double* rx=NULL;
  double* ru_x=NULL;
  /* conjugate function & abscissa */
  double* s=NULL; 
  double* u_star_s=NULL; 
  /* temporay pointers */       
  double* tmprx=NULL;
  double* tmpru_x=NULL; 
  ...
  {
    InterfError("linearlt: You must give 2 input arguments\n");
    return;
  }
  ...
  {
    InterfError("linearlt: You must give 2 or 4 output arguments\n");
    return;
  }
  ...
  {
    InterfError("linearlt: The abscissa: x  must be a numeric matrix\n");
    return;
  }
  if(!MatrixIsReal(Mx))
  {
    InterfError("linearlt: The abscissa: x must be a real matrix\n");
    return;
  } 
  ...
  {
    InterfError("linearlt: The function: u_x must be a numeric matrix\n");
    return;
  }
  if(!MatrixIsReal(Mu_x))
  {
    InterfError("linearlt: The function: u_x must be a real matrix\n");
    return;
  } 
  /* Check the size of the input function u_x */
  if(N!=(int)MAX(MatrixGetWidth(Mu_x),MatrixGetHeight(Mu_x)))
  {
    InterfError("linearlt: The abscissa: x and the function: u_x must be of the same size\n");
    return;
  } 
  ...
  {
    InterfError("linearlt: malloc error\n");
    return; 
  }               
  ...
  {
    InterfError("linearlt: Call of the C function MFAM_bb_concave_hull failed\n");
    return; 
  }
      
  /* Create the output Matrix */  
  Ms=MatrixCreate(1,M,"real");
  Mu_star_s=MatrixCreate(1,M,"real");
  ...
  /* Call of the C function */  
  if(MFAM_llt(M,tmprx,tmpru_x,s,u_star_s)==0) 
  { 
    InterfError("linearlt: Call of the C function MFAM_llt failed\n"); 
    return;
  }
