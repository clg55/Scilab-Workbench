...
c       --   subvariable critev(sim) --
        ...
        l4e22 = sadr(il4e22+4)
c      
c       --   subvariable ncblk(sim) --
        il4e23=iadr(l4+istk(il4+24)-1)
        l4e23 = sadr(il4e23+4)
        ncblk=stk(l4e23)
c      
c       --   subvariable nxblk(sim) --
...
c       --   subvariable iord(sim) --
        ...
        n4e29 = istk(il4e29+1)*istk(il4e29+2)
        l4e29 = sadr(il4e29+4)
c      
...
c        endif
        ...
        if (m4e16.ne.2.and.n4e16.ne.0) then
          call error(42)
          return
        endif
        if (m4e17.ne.2.and.n4e17.ne.0) then
          call error(42)
          return
        endif
...
c        call entier(n4e5,stk(l4e5),istk(iadr(l4e5)))
        ...
        call entier(n4e16*m4e16,stk(l4e16),istk(iadr(l4e16)))
        call entier(n4e17*m4e17,stk(l4e17),istk(iadr(l4e17)))
        call entier(n4e18*m4e18,stk(l4e18),istk(iadr(l4e18)))
        call entier(n4e19,stk(l4e19),istk(iadr(l4e19)))
        call entier(n4e20,stk(l4e20),istk(iadr(l4e20)))
        call entier(n4e21,stk(l4e21),istk(iadr(l4e21)))
        call entier(n4e22,stk(l4e22),istk(iadr(l4e22)))
        call entier(1,stk(l4e23),istk(iadr(l4e23)))
        call entier(1,stk(l4e24),istk(iadr(l4e24)))
        ...
        call entier(n4e29,stk(l4e29),istk(iadr(l4e29)))
c
        if(rhs.eq.5) then
           simpar(1) = 1.d-4
           simpar(2) = 1.d-6
...
c        ilclk=iadr(l4e11)
        ...
        nn42=ncst+ndst+nrpar+nout+22+ncst*max(16,ncst + 9)+3*ng 
        lw42=lw
        lw=lw+nn42
        nn43=nlab +  ninplnk + noutlnk + nlnkptr + 
     &       8*(nblk+1) + nipar  + nzord + noord +
     &        20 + ncst + 2*ng
        lw43=lw
        lw=lw+nn43
        err=lw-lstk(bot)
...
c
     ...
     $       istk(iadr(l4e16)),n4e16,istk(iadr(l4e17)),
     $       istk(iadr(l4e18)),istk(iadr(l4e19)),n4e19,
     $       istk(iadr(l4e29)),n4e29,
     $       istk(iadr(l4e20)),n4e20,istk(iadr(l4e21)),n4e21,
     $       istk(iadr(l4e22)),istk(iadr(l4e23)),istk(iadr(l4e24)),
     $       istk(iadr(l4e25)),istk(iadr(l4e26)),istk(iadr(l4e27)),
     $       n4e27,simpar,stk(lw42),stk(lw43),flag,ierr)
        idb=0
        if (ierr .gt. 0 ) then
           if(ierr.eq.1) then
