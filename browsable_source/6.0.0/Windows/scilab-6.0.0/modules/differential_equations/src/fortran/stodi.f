...
c-----------------------------------------------------------------------
      if (m .ne. 0) crate = max(0.20d+0*crate,del/delp)
      dcon = del*min(1.0d+0,1.50d+0*crate)/(tesco(2,nq)*conit)
      if (dcon .le. 1.0d+0) go to 460
      m = m + 1
...
c-----------------------------------------------------------------------
 ...
 445    continue
      ...
      if (abs(h) .le. hmin*1.000010d+0) go to 450
...
c-----------------------------------------------------------------------
 ...
 620  if ((kflag .eq. 0) .and. (rh .lt. 1.10d+0)) go to 610
      if (kflag .le. -2) rh = min(rh,0.10d+0)
c-----------------------------------------------------------------------
