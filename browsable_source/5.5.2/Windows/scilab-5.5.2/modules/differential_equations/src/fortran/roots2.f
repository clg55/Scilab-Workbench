      subroutine roots2(ng, hmin, jflag, x0, x1, g0, g1, gx, x, jroot)
clll. optimize
...
c repeat until the first root in the interval is found.  loop point. ---
 ...
 180  if((abs(g0(imax)).eq.zero).or.(abs(g1(imax)).eq.zero)) then
...
c check to see in which interval g changes sign. -----------------------
 200  imxold = imax
      ...
      zroot = .false.
      do 220 i = 1,ng
         if ((jroot(i) .eq. 1).and.((abs(gx(i)) .gt. zero))) iunstuck=i
