     ...
     char *loaded_files[];
{
  ...
  if ( strncmp(loaded_files[0],"scilab",6) !=0)
	   ...
	   && strstr(loaded_files[0],SHARED_SUF)!= NULL)
	{
	  ...
	  if (debug) sciprint("Loading shared executable %s\r\n",loaded_files[0]);
	}
      else 
	{
	  int rep;
 ...
 char *tmp_file;
{
  ...
  libs=getenv("SYSLIBS");
   /** XXXXX **/
  if (debug) sciprint("linking files ");
  while ( loaded_files[i] != NULL) 
    {
      if (debug) sciprint("%s ",loaded_files[i]);
      i++;
    }

  if (debug) sciprint(" to create a shared executable\r\n");
  count++;
