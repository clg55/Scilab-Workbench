     ...
     integer *h;
{
  ...
  if (sciGetwresize () == 1)
    ...
    {
      hRgn =
	CreateRectRgn ((int) *x - GetScrollPos (ScilabXgc->CWindow, SB_HORZ),
		       (int) *y - GetScrollPos (ScilabXgc->CWindow, SB_VERT),
		       (int) *x - GetScrollPos (ScilabXgc->CWindow,
						SB_HORZ) + *w,
		       (int) *y - GetScrollPos (ScilabXgc->CWindow,
						SB_VERT) + *h);
      ScilabXgc->CurClipRegion[0] = *x;	// - GetScrollPos(ScilabXgc->CWindow, SB_HORZ); a retirer
      ScilabXgc->CurClipRegion[1] = *y;	// - GetScrollPos(ScilabXgc->CWindow, SB_VERT); a retirer
      ScilabXgc->CurClipRegion[2] = *w;
      ScilabXgc->CurClipRegion[3] = *h;
      hdc = GetDC (ScilabXgc->CWindow);
      SelectClipRgn (hdc, hRgn);
