...
arglist = [];
for i = 4:nargin
  arglist = arglist+',p'+string(i-3);
end
if nargin<3 then B = 200; end
if min(size(x))==1 then   x = x(:); end
...
colWiseFun = theta=='mean'|theta=='std'|theta=='median'|theta=='quantile';
...
if (nx==1)&colWiseFun then
  Bchunk = ceil(40000/n);
  i = 1;
  while i<B then
    Bnext = min(B-i,Bchunk);
    xb = rboot(x,Bnext);
    y(:,i+(1:Bnext)) = evstr(evalstring);
    i = i+Bnext;
  end
else
  for i = 2:B
