...
mode(0);
num=x_message([' ';
	'Scigraph is a scilab vizualisation toolbox for Metanet graphs';
...
file('close',inin);
La=['Abbesses','Alesia','Alexandre Dumas','Alfort-Ecole Veterinaire','Alma-Marceau','Anatole-France','Anvers','Argentine','Arts et Metiers','Auber','Aubervilliers-Pantin','Avron'];
Lb=['Balard','Barbes-Rochechouart','Bastille','Bel air','Belleville','Berault','Bercy','Billancourt','Bir-Hakeim','Blanche','Boissiere','Bolivar','Bonne-Nouvelle','Botzaris','Boucicaut','Boulets - Montreuil','Boulevard Massena','Boulevard Victor','Boulogne - Jean Jaures','Boulogne - Pont de saint cloud','Bourse','Breguet-Sabin','Brochant','Buttes-Chaumont','Buzenval'];
Lc=['Cadet','Cambronne','Campo-Formio','Cardinal Lemoine','Carrefour Pleyel','Censier-Daubenton','Chambre des deputes','Champ-de-Mars','champs-Elysees Clemenceau','Chardon Lagache','Charenton-Ecoles','Ch. de Gaulle-Etoile','Charles Michels','Charonne','Chateau d eau','Chateau de Vincennes','Chateau Landon','Chateau-Rouge','Chatelet','Chatelet-les-Halles','Chatillon-Montrouge','Chaussee d Antin','Chemin Vert','Chevaleret','Cite','Cite Universitaire','Colonel Fabien','Commerce','Concorde'];
Lcc=['Convention','Corentin-Cariou','Corentin-Celton','Corvisart','Courcelles','Couronnes','Creteil-l Echat','Creteil Prefecture','Creteil Universite','Crimee','Croix de Chavaux'];
Ld=['Danube','Daumesnil','Denfert-Rochereau','Dugommier','Dupleix','Duroc'];
Le=['Ecole Militaire','Edgar Quinet','Eglise d Auteuil','Eglise de Pantin','Emile Zola','Etienne Marcel','Europe','Exelmans'];
Lf=['Faidherbe-Chaligny','Falguiere','Felix Faure','Filles du Calvaire','Fort d Aubervilliers','Franklin D Roosevelt'];
Lg=['Gabriel Peri','Gaite','Gallieni','Gambetta','Gare d Austerlitz','Gare de l Est','Gare de lyon','Gare du Nord','Garibaldi','Gentilly','George V','Glaciere','Goncourt','Guy Moquet'];
...
Lk=['Kleber'];
Ll=['La Chapelle','La Defense','La Fourche','Lamarck-Caulaincourt','La Motte Piquet','La Muette','Laplace','La Plaine Voyageurs','Latour Maubourg','Laumiere','Ledru Rollin','Le Kremlin Bicetre','Le Peletier','Les Gobelins','Les Halles','Les Sablons','Liberte','Liege','Louis Blanc','Louise Michel','Lourmel','Louvre','Luxembourg'];
Lm=['Mabillon','Madeleine','Mairie de clichy','Mairie de Montreuil','Mairie de St Ouen','Mairie des lilas','Mairie d Issy','Mairie d Ivry','Maison Blanche','Maison Alfort Les Julliottes','Maison Alfort Stade','Malakoff Plateau de Vanves','Malakoff Rue Etienne Dolet','Malesherbes','Maraichers','Marcadet Poissonniers','Marcel Sembat','Marx Dormoy','Maubert Mutualite','Menilmontant','Michel Ange Auteuil','Michel Ange Molitor','Michel Bizot','Mirabeau','Miromesnil','Monceau','Monge','Montgallet'];
...
Lp=['Palais Royal','Parmentier','Passy','Pasteur','Pelleport','Pereire','Pere Lachaise','Pernety','Philippe Auguste','Picpus','Pierre Curie','Pigalle','Place de Clichy','Place des Fetes','Place d Italie','Plaisance','Poissoniere','Pont de l Alma','Pont de Levallois Becon','Pont de Neuilly','Pont de Sevres','Pont Marie','Pont Neuf','Porte Dauphine','Porte d Auteuil','Porte de Bagnolet','Porte de Champeret','Porte de Charenton','Porte de Choisy','Porte de Clichy','Porte de Clignancourt'];
Lpp=['Porte de la Chapelle','Porte de la Vilette','Porte de Montreuil','Porte de Pantin','Porte de St Cloud','Porte de St Ouen','Porte des Lilas','Porte de Vanves','Porte de Versailles','Porte de Vincennes','Porte d Italie','Porte d Ivry','Porte Doree','Porte d Orleans','Porte Maillot','Port Royal','Pre St Gervais','Pyramides','Pyrenees'];
Lq=['Quai de la Gare','Quai de la Rapee','Quai d Orsay','Quatre Septembre'];
Lr=['Rambuteau','Ranelagh','Raspail','Reaumur Sebastopol','Rennes','Republique','Reuilly Diderot','Richard Lenoir','Richelieu Drouot','Riquet','Robespierre','Rome','Rue de la Pompe','Rue du Bac','Rue Montmartre'];
Ls=['Saint Ambroise','Saint Augustin','Saint Denis Basilique','Saint Denis Porte de Paris','Saint Fargeau','Saint Francois Xavier','Saint Georges','Saint Germain des Pres','Saint Jacques','Saint Lazare','Saint Mande Tourelle','Saint Marcel','Saint Maur','Saint Michel','Saint Paul','Saint Philippe du Roule','Saint Placide','Saint Sebastien Froissard','Saint Sulpice','Segur','Sentier','Sevres Babylone','Sevres Lecourbe','Simplon','Solferino','Stalingrad','Strasbourg St Denis','Sully Morland'];
...
g=load_graph(SCI+'/demos/metanet/paris2');
// scigraph y axis is not as metanet y axis 
z=g('node_y');
g('node_y')= maxi(z)-z;
show_scigraph(g,'rep',[700,700],[700,700]);
...
ppp=nodes_2_path(ttt,g);
show_scig_arcs(ppp);
show_scig_nodes(ttt,'sup');
name=g('node_name');
road=name(ttt)';
//--------------
kk=size(road);
...
// show the route 
xset('font',2,1);
xset('pattern',xget('lastpattern')+1);
for i=[1,kk(1)]
    xstring(g('node_x')(ttt(i))+10,g('node_y')(ttt(i)),routef(i))
end
//     
