...
static char szAppName[] = "SciHelp" ;
static HWND hDlgModeless ;

static void ReadIni();
static void WriteIni();
static int Winx,Winy,Winh,Winw;
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  static char      szFileName[_MAX_PATH],*loc ;
  ...
  switch (iMsg)
...
#ifdef __GNUC__
      loc = strtok(szFileName , " ");
      loc = strtok(NULL , " ");
#else
      loc = szFileName ;
#endif 
      if ( loc != (char *) 0 && strlen (loc) > 0)
	{
	  GetFileTitle (loc, szTitleName,sizeof (szTitleName)) ;
	  if (!PopFileRead (hwndEdit, loc ))
      ...
      if (lParam && LOWORD (wParam) == EDITID)
	{
	  switch (HIWORD (wParam))
	    {
	    case EN_UPDATE :
	      bNeedSave = FALSE ; /** JPC I don't want xless to change 
				    scilab mans : TRUE ; **/
	      return 0 ;
	...
	case IDM_FIND :
	  SendMessage (hwndEdit, EM_GETSEL, 0,
		       (LPARAM) &iOffset) ;
...
{
  char profile[80];
  wsprintf(profile, "%d %d", Winw,Winh);
  WritePrivateProfileString(section, "HelpSize", profile, file);
  return;
}
...
void ReadIni()
{
  ...
  Winx = CW_USEDEFAULT;
  Winy = CW_USEDEFAULT;
  GetPrivateProfileString(section, "HelpSize", "", profile, 80, file);
  if ( (p = GetInt(profile, &Winw )) == NULL) Winw= CW_USEDEFAULT;
  if ( (p = GetInt(p, &Winh)) == NULL) Winh = CW_USEDEFAULT;
}
