...
public class ScicosParametersTest {
	...
	@Test
	public void checkBeansPropertyChangeEvent() throws IllegalArgumentException, IllegalAccessException, InvocationTargetException {
		ScicosParameters obj = new ScicosParameters();
		...
		for (int i = 0; i < DEFAULT_VALUES.length; i++) {
			...
			if (defaultValue.getType().equals(double.class)) {
				...
				Class<? extends Object> type = Array.get(array, 0).getClass();
				int arrayLength = Array.getLength(array);
				Object value = Array.newInstance(type, arrayLength + 1);
				...
				/*
				 * The latest value must be equals to the previous one
				 */
				Array.set(value, arrayLength, Array.get(array, arrayLength - 1));
		 ...
		 */
		assert receivedEvents.size() == DEFAULT_VALUES.length;
		for (int i = 0; i < DEFAULT_VALUES.length; i++) {
	...
	@Test
	public void checkBeansVetoableChangeEvent() throws IllegalArgumentException, IllegalAccessException {
		ScicosParameters obj = new ScicosParameters();
