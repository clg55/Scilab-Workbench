...
//
[n,n]=size(p);[lhs,rhs]=argn(0);
// t1
if rhs=2 then [u,p,u,np]=svd(p);
         else [u,p,u,np]=svd(p,tol);
end;
...
if rhs=2 then [u,sigma1,u,nq]=svd(q(1:np,1:np))
         else [u,sigma1,u,nq]=svd(q(1:np,1:np),tol)
end;
...
//
// t4
if rhs=2 then [u,q,u,n3]=svd(q(np+1:n,np+1:n));
         else [u,q,u,n3]=svd(q(np+1:n,np+1:n),tol);
end;
