...
c
     ...
     /    fpd,z,z1,test
c
 1000 format (/,4x,' nlis0       fpn=',d10.3,' d2=',d9.2,
     1             '  tmin=',d9.2,' tmax=',d9.2)
 1001 format (/,4x,' nlis0',3x,'fin sur tmin',8x,'pas',
     1         12x,'fonctions',5x,'derivees')
 ...
 1007 format (/,4x,' nlis0',10x,'tmin force a tmax')
 1008 format (/,4x,' nlis0',10x,'appel incoherent')
      if (n.gt.0 .and. fpn.lt.0.0d+0 .and. t.gt.0.0d+0
     1 .and. tmax.gt.0.0d+0 .and. amf.gt.0.0d+0
     1 .and. amd.gt.amf .and. amd.lt.1.0d+0) go to 5
      logic=6
      go to 999
    5 tesf=amf*fpn
      tesd=amd*fpn
...
c
      ta=t
      t=9.0d+0*tg
      z=fpn+3.0d+0*fp-4.0d+0*ffn/tg
      if(z.gt.0.0d+0) t=min(t,tg*max(1.0d+0,-fp/z))
      t=tg+t
...
c
  500 if(indica.le.0) then
          ta=t
          t=0.90d+0*tg+0.10d+0*td
          go to 900
      endif
      z=fp+fpa-3.0d+0*(fa-f)/(ta-t)
      z1=z*z-fp*fpa
      if(z1.lt.0.0d+0) then
          ta=t
          t=0.50d+0*(td+tg)
          go to 900
      endif
      if(t.lt.ta) z1=z-sqrt(z1)
      if(t.gt.ta) z1=z+sqrt(z1)
      z=fp/(fp+z1)
      z=t+z*(ta-t)
      ta=t
      test=0.10d+0*(td-tg)
