...
//
//r and  c can be scalar, polynomial or character string matrices.
//!
[lhs,rhs]=argn(0),
if rhs=1 then r=conj(c');end
nr=prod(size(r));r=matrix(r,1,nr)
nc=prod(size(c));c=matrix(c,nc,1)
if nr*nc=0 then a=[],return,end
if r(1)<>c(1) then error('conflit entre r(1) et c(1)'),end
