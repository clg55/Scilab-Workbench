...
C     Abort unless random number generator initialized
      IF (qrgnin()) GO TO 10
      call basout(io,wte,'ADVNST called before random number generator')
      return
   10 CALL getcgn(g)
C
      ib1 = a1
      ib2 = a2
      DO 20,i = 1,k
          ib1 = mltmod(ib1,ib1,m1,ierr)
          ib2 = mltmod(ib2,ib2,m2,ierr)
   20 CONTINUE
      CALL setsd(mltmod(ib1,cg1(g),m1,ierr),mltmod(ib2,cg2(g),m2,ierr))
C
