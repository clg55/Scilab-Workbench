...
dimsrhs=Operation("rc",list(Cste(double(tree.rhs(1).infer.dims(1))),Cste(double(tree.rhs(1).infer.dims(2)))),list())

inrhs=tree.rhs(1)

tmprhslist=list()
tmprhslist(1)=dimsrhs
if double(tree.rhs(1).infer.dims(1))<>1 then
  
  for kr=1:double(tree.rhs(1).infer.dims(1))-1
...
rhslist=list()
rhslist(1)=dimsrhs
if double(tree.rhs(1).infer.dims(2))<>1 then

  tmprhslist(1)=null()
  
  for ksz=lstsize(tmprhslist):-1:1
    inrhs=tmprhslist(ksz)
    for kr=1:double(tree.rhs(1).infer.dims(2))-1
