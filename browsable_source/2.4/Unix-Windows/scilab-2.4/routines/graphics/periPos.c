...
{ 
  if (ScilabGCPos.CurColorStatus == 1) 
    {
      ScilabGCPos.NumHidden3d = Max(0,Min(*num - 1,ScilabGCPos.Numcolors + 1));
    }
}
...
struct bgc { char *name ;
  ...
  ScilabGCTabPos[] = {
    ...
    {"dashes",C2F(setdashPos),C2F(getdashPos)},
     ...
     double *dv4;
{
  ...
  C2F(getdashPos)(&verbose,Dvalue,&Dnarg,vdouble);
  if ((int)  *iflag == 0 )
    {
  ...
  C2F(setdashPos)( Dvalue,PI0,PI0,PI0);
}
  ...
  if ((int)  *iflag == 0 )
    {
      ...
      NDvalue=(*style < 1) ?  Dvalue[0] : *style;
      C2F(setdashPos)(&NDvalue,PI0,PI0,PI0);
      ...
      for ( i=0 ; i < *n/2 ; i++) 
	{
	  NDvalue = style[i];
	  C2F(setdashPos)(&NDvalue,PI0,PI0,PI0);
	  C2F(setpatternPos)(&NDvalue,PI0,PI0,PI0);
  ...
  for ( i=0 ; i < *n ; i++) 
    {
      integer fvect,na=1;
      /** to fix the style */
      C2F(setdashPos)(&style[i],PI0,PI0,PI0);
...
{ integer verbose ,symb[2],Mnarg,Dnarg,Dvalue[10],NDvalue,i,close;
  ...
  for (i=0 ; i< *n ; i++)
	...
	{/** on utilise un style pointill\'e  **/
	  ...
	  C2F(setdashPos)(drawvect+i,PI0,PI0,PI0);
	  close = 0;
