...
c     
      ...
      if(it1.ne.it2.and.mn1.ne.0.and.mn2.ne.0) then
         top=top+1
         write(fct,'(''%i_bitor'')')
         call setfunnam(ids(1,pt+1),fct,8)
         fun=-1
         return
      endif
...
c     .  vector+const
         call genbitor(it1,mn1,istk(l2),0,istk(l1),1)
         lstk(top+1)=sadr(l1+memused(it1,mn1))
      elseif (mn1.eq.1) then
c     .  cst+vector
         call gencopy(it1,1,istk(l1),1,is,1)
         call gencopy(it1,mn2,istk(l2),1,istk(l1),1)
         call genbitor(it1,mn2,is,0,istk(l1),1)
...
c     .  vector+vector
         ...
         call genbitor(it1,mn1,istk(l2),1,istk(l1),1)
         lstk(top+1)=sadr(l1+memused(it1,mn1))
      endif
 999  return
      end
