...
c     .     at the top of the stack
            ...
            ll1=sadr(il1i+m1i+3)
            top=top2+1
            call dcopy(istk(il1i+m1i+2)-1,stk(ll1),1,stk(lstk(top)),1)
            do 16 k1=1,m1i
...
c     (matrix/vector extraction, method..)
      ...
      if(istk(il2i).eq.17.or.istk(il2i).eq.16) goto 40
...
c     move results at its correct location
      ...
      call dcopy(vol,stk(lstk(top)),1,stk(lstk(top-2)),1)
      top=top-2
...
c     nothing more to do here
 ...
 40   continue
c     mlist or tlist coded matrix extraction or other method
c     .  form the sublist
...
c     .  copy the fields
         l=lstk(top-n+1)
         call dcopy(istk(illist+n+2)-1,stk(llist),1,stk(l),1)
      else
c     .  L([....])
