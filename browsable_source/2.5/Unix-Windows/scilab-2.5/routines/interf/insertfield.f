...
c     .     copy the beginning of "tos" up to the beginning of the modified sub-list. 
            ltod=sadr(iltos)
            ltto=sadr(ilto)
            call dcopy(ltto-ltod,stk(ltod),1,stk(lrn),1)
...
c     .  added field
         lr=sadr(ilr+4+mto)
         call dcopy(volfrom,stk(lfromt),1,stk(lr),1)
c     .  copy old sublist fields
         lr=lr+volfrom
         call dcopy(volto,stk(lto),1,stk(lr),1)
         lr=lr+volto
...
c     .  copy the rest of data structure
         if(nind.gt.1) then
            ...
            call dcopy(ltos+voltos-ltto,stk(ltto),1,stk(lr),1)
            lr=lr+ltos+voltos-ltto
         endif
c     .  put the result in place
         lrn=sadr(iln)
c         call dcopy(lr-lrn,stk(lrn),1,stk(lstk(top)),1)
...
c     .        copy the beginning of "to" up to field to be replaced
               ltod=sadr(iltos)
               call dcopy(lr-ltod,stk(ltod),1,stk(lrn),1)
...
c     .     copy first n-1 elements
            l=sadr(il+2+mto)
            call dcopy(istk(il+n+1)-1,stk(lto),1,stk(l),1)
            l=l+istk(il+n+1)-1
c     .     dvol the size variation of modified sub-element (-old+new)
            dvol=(l-sadr(il))-(lfrom-sadr(ilto))
c     .     copy last elements
            call dcopy(istk(il+1+mto)-istk(il+1+n),stk(lfrom),1,
     $           stk(l),1)
...
c     .     copy the rest of data structure
            ind(nind)=ind(nind)+1
            ltto=sadr(subptr(iltos,ind,nind))
            call dcopy(ltos+voltos-ltto,stk(ltto),1,stk(l),1)
