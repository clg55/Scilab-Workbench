...
static void gdImageBrushApply _PARAMS((gdImagePtr im, int x, int y));
static void gdImageTileApply _PARAMS((gdImagePtr im, int x, int y));
     ...
     int close;
{
  ...
  if (close) 
    gdImageLine(imv,X[n-1],X[n-1],X[0],Y[0],c);
...
static void dashedSet(im, x, y, color, onP, dashStepP)
     gdImagePtr im;
     int x;
     int y;
     int color;
