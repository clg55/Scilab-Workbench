...
c     .           not enought memory, realloc
                  mem=lstk(gbot)-lstk(isiz+2)+v+1
                  call reallocglobal(mem)
                  if(err.gt.0) return
                  ls=lstk(kg+1)
                  ll = ls + v - vk
               endif
               call dcopy(lstk(gtop+1)-lstk(kg+1),stk(ls),-1,stk(ll),-1)
c     .        update pointer               
            else
...
c     copy new value
 25   call dcopy(v,stk(lstk(top)),1,stk(lstk(kg)),1)
      fin=kg
      top=top-1
      return
      end
...
c
      ...
      offset=offset+1
      call dcopy(l,stk(lstk(isiz+2)),1,stk(offset),1)
