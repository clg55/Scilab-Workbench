...
are evaluated but are not displayed on screen. Scilab is case 
sensitive now (Version 2.0 was not case sensitive). 
...
\verbatok{macros/make_macro.sci}
\end{verbatim}
...
For example, in Scilab linear systems are treated as lists.
...
\verbatok{\Diary syslin.dia}
\end{verbatim}

As can be seen by the above Scilab session the list {\tt h} begins
...
($\dot{x}=Ax+Bu$, $y=Cx+Du$, $x(0)=x_0$).  Finally, the last element
...
the manipulation of systems in Scilab  is that a system can
...
\put(180,270){\makebox(0,0)[lb]{\tt s1*s2}}
\put(9,270){\circle{2}}
...
\put(180,140){\makebox(0,0)[lb]{\verb![s1,s2]!}}
\put(49,160){\circle{2}}
\put(49,120){\circle{2}}
...
\put(180,-40){\makebox(0,0)[lb]{\verb! s1/.s2!}}
\put(69,-20){\circle{2}}
