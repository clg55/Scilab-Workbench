...
c     row concatenation
         ...
         do 20 i=1,ma
            ...
            call dcopy(inda(i),ar(ia),1,cr(ic),1)
            if(ita.eq.0) then
               call dset(inda(i),0.0d0,ci(ic),1)
            else
               call dcopy(inda(i),ai(ia),1,ci(ic),1)
            endif
            ic=ic+inda(i)
            ia=ia+inda(i)
            if (indb(i).gt.0) then
               call dcopy(indb(i),br(ib),1,cr(ic),1)
               if(itb.eq.0) then
                  call dset(indb(i),0.0d0,ci(ic),1)
               else
                  call dcopy(indb(i),bi(ib),1,ci(ic),1)
               endif
               do 10 k=1,indb(i)
...
c     column concatenation
        ...
        call dcopy(nela,ar,1,cr,1)
        if(ita.eq.0) then
           call dset(nela,0.0d0,ci,1)
        else
           call dcopy(nela,ai,1,ci,1)
        endif
        call dcopy(nelb,br,1,cr(nela+1),1)
        if(itb.eq.0) then
           call dset(nelb,0.0d0,ci(nela+1),1)
        else
           call dcopy(nelb,bi,1,ci(nela+1),1)
        endif
        nelc=nela+nelb
      endif
      return 
