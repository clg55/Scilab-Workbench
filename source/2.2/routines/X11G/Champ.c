...
/*-----------------------------------------------------------------
   champ_(x,y,fx,fy,strflag,vrect,arfact)
...
champg(name,colored,x,y,fx,fy,n1,n2,strflag,brect,arfact,lstr)
     double x[],y[],fx[],fy[],brect[],*arfact;
     integer *n1,*n2;
     char strflag[];
     integer lstr,colored;
     char name[];
{
  static char logflag[]="nn";
  static integer aaint[]={2,10,2,10};
  double FRect[4],scx,scy,xofset,yofset, maxx;
  integer IRect[4],IRect1[4],err=0,*xm,*ym,*zm,i,j,job=1,n,na;
  ...
  if ( colored == 0 ) 
      ...
      for ( i = 0 ; i < (*n1)*(*n2) ; i++)
	{
	  ...
	  xm[1+2*j]= sfx*fx[i]/2+xm[2*i];
	  xm[2*j]  = -sfx*fx[i]/2+xm[2*i];
	  ym[1+2*j]= -sfy*fy[i]/2+ym[2*i];
	  ym[2*j]  = sfy*fy[i]/2+ym[2*i];
      ...
      for ( i = 0 ; i < (*n1)*(*n2) ; i++)
	{
	  ...
	  xm[1+2*j]= sfx*fx[i]/(2*nor)+xm[2*i];
	  xm[2*j]  = -sfx*fx[i]/(2*nor)+xm[2*i];
	  ym[1+2*j]= -sfy*fy[i]/(2*nor)+ym[2*i];
	  ym[2*j]  = sfy*fy[i]/(2*nor)+ym[2*i];
	  clip_line(xm[2*j],ym[2*j],xm[2*j+1],ym[2*j+1],&x1n,&y1n,&x2n,&y2n,&flag1);
	  if (flag1 !=0)
	    {
	      if (flag1==1||flag1==3) { xm[2*j]=x1n;ym[2*j]=y1n;};
...
C2F(champ)(x,y,fx,fy,n1,n2,strflag,brect,arfact,lstr)
     double x[],y[],fx[],fy[],brect[],*arfact;
     integer *n1,*n2;
     char strflag[];
     integer lstr;
...
C2F(champ1)(x,y,fx,fy,n1,n2,strflag,brect,arfact,lstr)
     double x[],y[],fx[],fy[],brect[],*arfact;
     integer *n1,*n2;
     char strflag[];
     integer lstr;
