...
c     .  [[];b]
     ...
     $        ,1,stk(lstk(top)),1)
      ...
      mn=m*n1
      if(n1.eq.1) then
         call gencopy(it1,mn2,istk(l2),1,istk(l1+mn1),1)
         lstk(top+1)=sadr(l1+memused(it1,mn))
         istk(il1+1)=m
         return
      endif

      lw1=max(iadr(lw),l1+mn)
      lw2=lw1+mn1
...
c     
      call genmcopy(it1,istk(lw1),m1,istk(l1),m,m1,n1)
      call genmcopy(it1,istk(lw2),m2,istk(l1+m1),m,m2,n1)
