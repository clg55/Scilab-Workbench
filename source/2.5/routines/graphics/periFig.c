...
static void C2F(FileInitXfig) _PARAMS((void));
 ...
 *value=ScilabGCXfig.CurDashStyle+1;
 if ( *value == 1) 
...
{ 
  if (ScilabGCXfig.CurColorStatus == 1) 
    {
      ScilabGCXfig.NumHidden3d = Max(0,Min(*num - 1,ScilabGCXfig.Numcolors + 1));
    }
}
...
void C2F(fillarcsXfig)(str, vects, fillvect, n, v5, v6, v7, dx1, dx2, dx3, dx4)
     char *str;
     integer *vects;
     integer *fillvect;
     integer *n;
...
void C2F(drawarcXfig)(str, x, y, width, height, angle1, angle2, dx1, dx2, dx3, dx4)
     char *str;
     integer *x;
     integer *y;
     integer *width;
...
void C2F(fillarcXfig)(str, x, y, width, height, angle1, angle2, dx1, dx2, dx3, dx4)
     char *str;
     integer *x;
     integer *y;
     integer *width;
...
{ integer verbose ,symb[2],Mnarg,Dnarg,Dvalue[10],NDvalue,i,close;
  ...
  for (i=0 ; i< *n ; i++)
	...
	{/** on utilise un style pointill\'e  **/
	  C2F(setdashXfig)(drawvect+i,PI0,PI0,PI0);
	  close = 0;
...
void set_dash_or_color(dash,l_style,style_val,color)
     int *l_style,*style_val,*color,dash;
{
  if (  ScilabGCXfig.CurColorStatus == 1) 
    {
      int m;
      *color = dash-1 ; /** color value **/
      if (  CheckColormap(&m) == 1) 
	{
