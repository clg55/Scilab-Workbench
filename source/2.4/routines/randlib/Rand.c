...
{ 
  ...
  if ( strcmp(cstk(ls),"bet")==0) 
    ...
    {
      int nn,un=1,work,mp,parm,ierr,n1p1,lr1,j,icur;
      if ( suite != 3 || ResL*ResC != 1)
	...
	{ sciprint("Missing P matrix and X0 for Markov chain\r\n");Error(999);return 0;}
      ...
      if ( m2*n2 != 1 ) { sciprint("X0 must be a saclar\r\n");Error(999);return 0;}
      if ( *istk(lb)-1 < 0 || *istk(lb)-1 >= n1 ) 
	{
	  sciprint("X0 must be in the range [1,%d]\r\n",n1);
	  Error(999);return 0;
	}
      CreateVar(suite+2,"i",&un,&nn,&lr);
      n1p1=n1+1;
